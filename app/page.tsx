...
function BigMetric({ icon: Icon, label, value, sub, change, tone = "cyan" }: { icon: IconType; label: string; value: string; sub?: string; change?: number; tone?: "cyan" | "green" | "amber" | "red" }) {
  ...
  return <Panel className="relative overflow-hidden p-5"><div className={cn("absolute right-5 top-5 rounded-xl bg-gradient-to-br p-3", toneClass)}><Icon className="h-5 w-5" /></div><p className="text-[0.68rem] font-black uppercase tracking-[0.34em] text-white/45">{label}</p><p className="mt-5 text-3xl font-black tracking-tight">{value}</p><div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-white/55">{typeof change === "number" && Number.isFinite(change) ? <MoveBadge value={change} /> : null}{sub ? <span>{sub}</span> : null}</div></Panel>;
}
...
function BarMiniChart({ values }: { values: number[] }) {
  ...
  return <div className="flex h-36 items-end gap-1.5 rounded-xl border border-white/5 bg-black/15 p-4">{clean.slice(-44).map((value, index) => <div key={index} className="flex-1 rounded-t-md bg-cyan-200/60" style={{ height: `${Math.max(4, (value / max) * 100)}%` }} />)}</div>;
}
function ViewHeader({ icon: Icon, eyebrow, title, description, right }: { icon: IconType; eyebrow: string; title: string; description: string; right?: React.ReactNode }) {
  return <div className="mb-7 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><Pill className="mb-3 border-cyan-200/25 bg-cyan-300/10 text-cyan-100"><Icon className="h-3.5 w-3.5" />{eyebrow}</Pill><h1 className="max-w-5xl text-4xl font-black leading-[0.95] tracking-tight md:text-7xl">{title}</h1><p className="mt-4 max-w-3xl text-base leading-7 text-white/72">{description}</p></div>{right ? <div className="flex flex-wrap gap-2">{right}</div> : null}</div>;
}
