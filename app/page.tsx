...
export default function App() {
  ...
  const valueCaptureStatus = valueCaptureRatio > 0 && valueCaptureRatio < 12 ? "Undervalued" : valueCaptureRatio > 0 && valueCaptureRatio < 20 ? "Fair value" : valueCaptureRatio > 0 ? "Risky" : "Buyback data pending";
  const historyPrices = historyPrices;
  const historyHigh = historyPrices.length ? Math.max(...historyPrices) : 0;
  const historyLow = historyPrices.length ? Math.min(...historyPrices) : 0;
  const historyMid = historyPrices.length ? (historyHigh + historyLow) / 2 : 0;
