...
type NftSale = { id: string; name: string; price: string; usd?: string; time: string; image?: string; url?: string; imageMode?: "artwork" | "preview"; priceSource?: "api" | "html" | "missing" };
...
function normalizeNftSale(raw: any, index: number): NftSale {
  ...
  const rawPrice = typeof raw.price === "string" ? raw.price : raw.priceLabel || "";
  const price = !rawPrice || rawPrice.startsWith("--") ? "Price n/a" : rawPrice.replace(/WHYPE/g, "HYPE");
  const imageMode = raw.imageMode || raw.image_mode || (raw.imageStatus === "item_page" ? "preview" : "artwork");
  const priceSource = raw.priceSource || raw.price_source || (price === "Price n/a" ? "missing" : "api");
  return { id, name, price, usd: raw.usd || raw.usdPrice || raw.usd_price || "", time, image, url: raw.url || raw.permalink || nft.permalink || nft.opensea_url || OPENSEA_COLLECTION_URL, imageMode, priceSource };
}
...
function riskTone(value: number) { return value >= 80 ? "text-red-200 bg-red-400/15 border-red-300/20" : value >= 65 ? "text-amber-200 bg-amber-300/15 border-amber-200/20" : value >= 45 ? "text-cyan-100 bg-cyan-300/10 border-cyan-200/15" : "text-emerald-200 bg-emerald-300/10 border-emerald-200/20"; }
...
function MoveBadge({ value }: { value: number }) {
  ...
  return <span className={cn("inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-extrabold", up ? "bg-emerald-400/15 text-emerald-100" : "bg-red-400/15 text-red-100")}><Icon className="h-3.5 w-3.5" />{formatPercent(value, 2)} 24h</span>;
}
function BigMetric({ icon: Icon, label, value, sub, change, tone = "cyan" }: { icon: IconType; label: string; value: string; sub?: string; change?: number; tone?: "cyan" | "green" | "amber" | "red" }) {
  ...
  return <Panel className="relative overflow-hidden p-5"><div className={cn("absolute right-5 top-5 rounded-2xl bg-gradient-to-br p-3", toneClass)}><Icon className="h-5 w-5" /></div><p className="text-[0.68rem] font-black uppercase tracking-[0.34em] text-white/45">{label}</p><p className="mt-5 text-3xl font-black tracking-tight">{value}</p><div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-white/55">{typeof change === "number" && Number.isFinite(change) ? <MoveBadge value={change} /> : null}{sub ? <span>{sub}</span> : null}</div></Panel>;
}
...
function BarMiniChart({ values }: { values: number[] }) {
  ...
  return <div className="flex h-36 items-end gap-1.5 rounded-2xl border border-white/8 bg-black/15 p-4">{clean.slice(-44).map((value, index) => <div key={index} className="flex-1 rounded-t-md bg-cyan-200/60" style={{ height: `${Math.max(4, (value / max) * 100)}%` }} />)}</div>;
}
function ViewHeader({ icon: Icon, eyebrow, title, description, right }: { icon: IconType; eyebrow: string; title: string; description: string; right?: React.ReactNode }) {
  return <div className="mb-7 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><Pill className="mb-3 border-cyan-200/25 bg-cyan-300/10 text-cyan-100"><Icon className="h-3.5 w-3.5" />{eyebrow}</Pill><h1 className="max-w-5xl text-5xl font-black leading-[0.95] tracking-tight md:text-7xl">{title}</h1><p className="mt-4 max-w-3xl text-base leading-7 text-white/72">{description}</p></div>{right ? <div className="flex flex-wrap gap-2">{right}</div> : null}</div>;
}
function NftEmptyState({ message }: { message: string }) { return <Panel className="p-8 text-center"><p className="text-lg font-black">No live NFT sales loaded</p><p className="mt-2 text-sm text-white/55">{message}</p><a href={OPENSEA_COLLECTION_URL} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-full border border-white/15 px-4 py-2 text-sm font-bold text-cyan-100">OpenSea <ExternalLink className="h-3.5 w-3.5" /></a></Panel>; }
function NftSkeleton() { return <div className="animate-pulse rounded-3xl border border-white/10 bg-white/[0.04]"><div className="aspect-square rounded-t-3xl bg-white/8"/><div className="h-20 p-4"><div className="h-4 w-24 rounded bg-white/10"/><div className="mt-3 h-3 w-14 rounded bg-white/10"/></div></div>; }
function NftSaleCard({ sale }: { sale: NftSale }) {
  const [broken, setBroken] = useState(false);
  const hasImage = Boolean(sale.image && !broken);
  const previewMode = sale.imageMode === "preview";
  const priceIsMissing = !sale.price || sale.price === "Price n/a";
  ...
  return <a href={sale.url || OPENSEA_COLLECTION_URL} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-3xl border border-white/12 bg-[#062820] shadow-xl shadow-black/20 transition hover:-translate-y-0.5 hover:border-cyan-200/35">
    <div className="relative aspect-square overflow-hidden bg-[#0b332b]">
      {hasImage ? (
        previewMode ? (
          <img src={sale.image} alt={sale.name} onError={() => setBroken(true)} loading="lazy" decoding="async" referrerPolicy="no-referrer" className="h-full max-w-none object-cover transition duration-500 group-hover:scale-[1.03]" style={{ width: "200%", objectPosition: "left center" }} />
        ) : (
          <img src={sale.image} alt={sale.name} onError={() => setBroken(true)} loading="lazy" decoding="async" referrerPolicy="no-referrer" className="h-full w-full object-cover object-center transition duration-500 group-hover:scale-[1.03]" />
        )
      ) : <div className="flex h-full w-full flex-col items-center justify-center gap-3 text-white/40"><ImageIcon className="h-10 w-10"/><span className="text-xs font-black uppercase tracking-[0.3em]">No image</span></div>}
    </div>
    <div className="flex min-h-20 items-center justify-between gap-3 p-4">
      <div className="min-w-0"><p className="truncate text-lg font-black">{sale.name}</p><p className="text-sm text-white/52">{sale.time}</p></div>
      <span className={cn("shrink-0 rounded-full border px-3 py-1 text-xs font-black", priceIsMissing ? "border-white/10 bg-white/[0.06] text-white/45" : "border-cyan-200/20 bg-cyan-300/12 text-cyan-100")}>{sale.price}</span>
    </div>
  </a>;
}
...
export default function App() {
  ...
  const sidebar = <aside className="fixed left-0 top-0 z-30 hidden h-screen w-64 border-r border-white/10 bg-[#031612]/95 p-4 backdrop-blur-xl lg:block"><div className="mb-8 flex items-center gap-3"><div className="rounded-2xl border border-cyan-200/20 bg-cyan-200/10 p-2"><Zap className="h-5 w-5 text-cyan-100" /></div><div><p className="text-lg font-black uppercase tracking-[0.28em]">Hypurr Scope</p><p className="text-xs text-white/55">HYPE market console</p></div></div><nav className="space-y-2">{NAV_ITEMS.map((item) => { const Icon = item.icon; const active = activeView === item.id; return <button key={item.id} onClick={() => setActiveView(item.id)} className={cn("flex w-full items-center gap-3 rounded-2xl border px-3 py-3 text-left transition", active ? "border-cyan-200/25 bg-cyan-200/10 text-white" : "border-transparent text-white/55 hover:bg-white/[0.04] hover:text-white") }><Icon className="h-5 w-5"/><span><span className="block text-sm font-black">{item.label}</span><span className="block text-xs text-white/45">{item.description}</span></span></button>; })}</nav><div className="absolute bottom-4 left-4 right-4 rounded-2xl border border-white/10 bg-white/[0.05] p-3 text-xs text-white/55"><div className="mb-2 flex items-center justify-between"><span>Refresh</span><RefreshCw className="h-3.5 w-3.5" /></div><p>Markets 20s · NFTs 120s · Tape 30s</p><p className="mt-1">Last update: {formatClock(lastUpdated)}</p></div></aside>;

  const overview = <div><ViewHeader icon={Gauge} eyebrow="Live HYPE console" title="HYPE price, buybacks, NFTs and market pressure" description="Focused on what matters: HYPE price, FDV, open interest, volume, Assistance Fund pressure, latest Hypurr sales, TWAP tape and ETF daily volume." right={<><StatusPill status={marketsStatus}/><StatusPill status={nftStatus}/></>} /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><BigMetric icon={hype.rawChange >= 0 ? ArrowUpRight : ArrowDownRight} label="HYPE price" value={hype.price} change={hype.rawChange} sub={`OI ${hype.oi}`} tone={hype.rawChange >= 0 ? "green" : "red"}/><BigMetric icon={Zap} label="FDV" value={hype.fdv || "$--"} sub="1B total supply"/><BigMetric icon={Activity} label="24h volume" value={hype.volumeLabel} sub={`Total tracked ${formatUsd(totalVolume)}`} tone="green"/><BigMetric icon={Flame} label="Buyback pressure" value={buybacks.estimatedBuybackUsd24hLabel || "Loading"} sub={buybacks.estimatedBuybackHype24hLabel || "Estimated / 24h"} tone="amber"/></div><div className="mt-5 grid gap-5 xl:grid-cols-[1.08fr_0.92fr]"><Panel className="p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-black">HYPE 24h chart</h2><p className="text-sm text-white/55">Live candles when available.</p></div><MoveBadge value={hype.rawChange}/></div><div className="h-[280px] rounded-3xl border border-white/8 bg-black/18 p-4"><Sparkline points={history.map((point) => point.price)} height={260} stroke={hype.rawChange >= 0 ? "#6ee7b7" : "#fda4af"}/></div><div className="mt-4 grid gap-3 md:grid-cols-3"><Panel className="p-4"><p className="text-xs font-black uppercase tracking-[0.28em] text-white/45">Perp Open Interest</p><p className="mt-3 text-2xl font-black">{hype.oi}</p><p className="text-xs text-white/55">{totalOi ? ((hype.rawOi / totalOi) * 100).toFixed(1) : "--"}% of tracked OI</p></Panel><Panel className="p-4"><p className="text-xs font-black uppercase tracking-[0.28em] text-white/45">Funding</p><p className="mt-3 text-2xl font-black text-emerald-200">{hype.funding}</p><p className="text-xs text-white/55">Longs pay if positive</p></Panel><Panel className="p-4"><p className="text-xs font-black uppercase tracking-[0.28em] text-white/45">Risk heat</p><p className="mt-3 text-2xl font-black">{hype.risk}/100</p><div className="mt-3 h-2 rounded-full bg-white/10"><div className="h-full rounded-full bg-cyan-200" style={{ width: `${hype.risk}%` }}/></div></Panel></div></Panel><Panel className="p-5"><div className="mb-4 flex items-start justify-between"><div><h2 className="text-xl font-black">Latest Hypurr NFT sales</h2><p className="text-sm text-white/55">NFT art only. Price badge uses sale-event data when available.</p></div><a href={OPENSEA_COLLECTION_URL} target="_blank" rel="noreferrer"><Pill>OpenSea <ExternalLink className="h-3.5 w-3.5"/></Pill></a></div><NftGrid sales={nftSales} loading={nftStatus === "loading"} message={nftMessage} compact max={4}/></Panel></div><div className="mt-5 grid gap-5 xl:grid-cols-3"><Panel className="p-5"><h2 className="text-xl font-black">HYPE TWAP tape</h2><p className="text-sm text-white/55">Recent HYPE flow clustering.</p><div className="mt-5 grid grid-cols-3 gap-3"><Panel className="p-3"><p className="text-xs text-white/45">Buy 10m</p><p className="mt-2 font-black text-emerald-100">{twapSummary?.buyLabel || "$--"}</p></Panel><Panel className="p-3"><p className="text-xs text-white/45">Sell 10m</p><p className="mt-2 font-black text-red-100">{twapSummary?.sellLabel || "$--"}</p></Panel><Panel className="p-3"><p className="text-xs text-white/45">Net</p><p className="mt-2 font-black">{twapSummary?.netSide || "--"} {twapSummary?.netLabel || "$--"}</p></Panel></div></Panel><Panel className="p-5"><h2 className="text-xl font-black">ETF daily volume</h2><p className="text-sm text-white/55">US/EU HYPE products.</p><div className="mt-4 space-y-2">{flows.slice(0, 3).map((row) => <div key={row.ticker} className="flex items-center justify-between rounded-2xl border border-white/8 bg-white/[0.04] p-3"><div><p className="font-black">{row.ticker}</p><p className="text-xs text-white/45">{row.venue}</p></div><div className="text-right"><p className="font-black">{row.dollarVolume || row.volume || "--"}</p><p className="text-xs text-white/45">daily value</p></div></div>)}</div></Panel><Panel className="p-5"><h2 className="text-xl font-black">HLP focus</h2><p className="text-sm text-white/55">Most useful vault block.</p><p className="mt-6 text-3xl font-black">{vaults[0]?.aum || "Loading"}</p><p className="mt-2 text-sm text-white/55">{vaults[0]?.name || "HLP"} · {vaults[0]?.apr || "APR loading"}</p></Panel></div></div>;

  const marketsView = <div><ViewHeader icon={BarChart3} eyebrow="Markets" title="HYPE + Hyperliquid perps" description="FDV, OI, funding, 24h volume, risk and relative market pressure." right={<StatusPill status={marketsStatus}/>} /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><BigMetric icon={Zap} label="HYPE FDV" value={hype.fdv || "$--"} sub="1B total supply"/><BigMetric icon={BarChart3} label="HYPE Volume" value={hype.volumeLabel} sub="24h notional"/><BigMetric icon={Activity} label="HYPE OI" value={hype.oi} sub={`${totalOi ? ((hype.rawOi / totalOi) * 100).toFixed(1) : "--"}% share`}/><BigMetric icon={Gauge} label="Risk heat" value={`${hype.risk}/100`} sub="move + OI + funding" tone="amber"/></div><Panel className="mt-5 p-5"><div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><h2 className="text-xl font-black">Market table</h2><div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/20 px-3 py-2"><Search className="h-4 w-4 text-white/40"/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search coin" className="bg-transparent text-sm outline-none placeholder:text-white/35"/></div></div><div className="overflow-x-auto"><table className="w-full min-w-[780px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.22em] text-white/40"><tr><th className="py-3">Coin</th><th>Price</th><th>24h</th><th>OI</th><th>Volume</th><th>Funding</th><th>FDV</th><th>Risk</th></tr></thead><tbody>{sortedMarkets.map((row) => <tr key={row.symbol} className="border-t border-white/8"><td className="py-3 font-black">{row.symbol}</td><td>{row.price}</td><td className={row.rawChange >= 0 ? "text-emerald-200" : "text-red-200"}>{row.change}</td><td>{row.oi}</td><td>{row.volumeLabel}</td><td>{row.funding}</td><td>{row.fdv || "--"}</td><td><span className={cn("rounded-full border px-2 py-1 text-xs font-black", riskTone(row.risk))}>{row.risk}</span></td></tr>)}</tbody></table></div></Panel><Panel className="mt-5 p-5"><h2 className="mb-4 text-xl font-black">Volume distribution</h2><BarMiniChart values={markets.slice(0, 24).map((row) => row.rawVolume)} /></Panel></div>;

  const buybacksView = <div><ViewHeader icon={Flame} eyebrow="Buybacks" title="Assistance Fund pressure" description="A transparent estimate of daily HYPE buyback pressure from Hyperliquid market volume. Exact purchases require an indexed onchain feed." right={<StatusPill status={buybackStatus}/>} /><div className="grid gap-4 md:grid-cols-3"><BigMetric icon={Flame} label="Estimated AF buy" value={buybacks.estimatedBuybackUsd24hLabel || "--"} sub="USD / 24h" tone="amber"/><BigMetric icon={Zap} label="HYPE equivalent" value={buybacks.estimatedBuybackHype24hLabel || "--"} sub="estimated"/><BigMetric icon={Activity} label="Total fees estimate" value={buybacks.totalFeeUsd24hLabel || "--"} sub="all tracked markets"/></div><Panel className="mt-5 p-6"><h2 className="text-xl font-black">Important</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-white/65">{buybacks.note || "This is an estimate. Do not treat it as exact onchain buyback accounting."}</p></Panel></div>;

  const buyerTwaps = twaps.filter((row) => row.side === "Buy");
  const sellerTwaps = twaps.filter((row) => row.side === "Sell");
  const renderTwapRows = (rows: TwapRow[], empty: string) => rows.length ? rows.map((row, index) => <div key={`${row.side}-${index}`} className="rounded-2xl border border-white/8 bg-white/[0.04] p-4">
    <div className="flex items-center justify-between"><span className={cn("rounded-full px-3 py-1 text-xs font-black", row.side === "Buy" ? "bg-emerald-400/15 text-emerald-100" : "bg-red-400/15 text-red-100")}>{row.side}</span><span className="font-black">{row.notional}</span></div>
    <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-white/55"><span>{row.slices} slices</span><span>{row.size}</span><span>{row.avgPrice}</span></div>
    <p className="mt-2 text-xs text-white/40">{row.confidence} confidence · last {row.lastTrade}</p>
  </div>) : <p className="rounded-2xl border border-white/8 bg-white/[0.035] p-4 text-sm text-white/55">{empty}</p>;

  const twapView = <div><ViewHeader icon={Clock3} eyebrow="HYPE TWAP" title="HYPE buyer/seller TWAP tape" description="HYPE-only tape built from recent public trades. It separates buyer pressure and seller pressure instead of mixing everything into one useless list." right={<StatusPill status={twapStatus}/>} /><div className="grid gap-4 md:grid-cols-3"><BigMetric icon={ArrowUpRight} label="Buyer flow 10m" value={twapSummary?.buyLabel || "$--"} sub="HYPE recent trades" tone="green"/><BigMetric icon={ArrowDownRight} label="Seller flow 10m" value={twapSummary?.sellLabel || "$--"} sub="HYPE recent trades" tone="red"/><BigMetric icon={Activity} label="Net flow" value={`${twapSummary?.netSide || "--"} ${twapSummary?.netLabel || "$--"}`} sub="10m window"/></div><div className="mt-5 grid gap-5 xl:grid-cols-2"><Panel className="p-5"><h2 className="mb-1 text-xl font-black text-emerald-100">Buyer TWAPs</h2><p className="mb-4 text-sm text-white/55">Clusters of repeated HYPE buys.</p><div className="space-y-3">{renderTwapRows(buyerTwaps, "No strong buyer TWAP-like cluster right now.")}</div></Panel><Panel className="p-5"><h2 className="mb-1 text-xl font-black text-red-100">Seller TWAPs</h2><p className="mb-4 text-sm text-white/55">Clusters of repeated HYPE sells.</p><div className="space-y-3">{renderTwapRows(sellerTwaps, "No strong seller TWAP-like cluster right now.")}</div></Panel></div><Panel className="mt-5 p-5"><h2 className="mb-4 text-xl font-black">Recent HYPE trades</h2><div className="max-h-[420px] overflow-auto rounded-2xl border border-white/8"><table className="w-full text-left text-sm"><thead className="sticky top-0 bg-[#06231f] text-xs uppercase tracking-[0.18em] text-white/40"><tr><th className="p-3">Side</th><th>Price</th><th>Size</th><th>Notional</th><th>Time</th></tr></thead><tbody>{trades.slice(0, 40).map((row) => <tr key={row.id} className="border-t border-white/8"><td className={cn("p-3 font-black", row.side === "Buy" ? "text-emerald-200" : "text-red-200")}>{row.side}</td><td>{row.price}</td><td>{row.size}</td><td>{row.notionalLabel}</td><td className="text-white/45">{row.timeLabel}</td></tr>)}</tbody></table></div></Panel></div>;

  const nftView = <div><ViewHeader icon={ImageIcon} eyebrow="Hypurr NFT Tape" title="Hypurr floor, stats and latest sales" description="Square artwork view. The price badge uses sale-event data only; otherwise it shows n/a instead of a fake price." right={<><StatusPill status={nftStatus}/><a href={OPENSEA_COLLECTION_URL} target="_blank" rel="noreferrer"><Pill>OpenSea <ExternalLink className="h-3.5 w-3.5"/></Pill></a></>} /><div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6"><BigMetric icon={ImageIcon} label="Floor" value={nftStats.floor}/><BigMetric icon={Activity} label="24h volume" value={nftStats.volume24h}/><BigMetric icon={BarChart3} label="Total volume" value={nftStats.totalVolume}/><BigMetric icon={Layers} label="Listed" value={nftStats.listed}/><BigMetric icon={Wallet} label="Owners" value={nftStats.owners}/><BigMetric icon={Zap} label="24h sales" value={nftStats.sales24h}/></div><Panel className="mt-5 p-5"><div className="mb-4"><h2 className="text-xl font-black">Latest sales</h2><p className="text-sm text-white/55">Updated {formatClock(nftLastUpdated)}. Refresh every 120 seconds.</p></div><NftGrid sales={nftSales} loading={nftStatus === "loading"} message={nftMessage} max={12}/></Panel></div>;

  const hlpView = <div><ViewHeader icon={Layers} eyebrow="HLP" title="HLP and major vault monitor" description="Focus on HLP first, then large vaults. Avoids noisy vault spam." right={<StatusPill status={vaultStatus}/>} /><div className="grid gap-4 md:grid-cols-3"><BigMetric icon={Layers} label="Top vault" value={vaults[0]?.name || "HLP"} sub={vaults[0]?.aum || "Loading"}/><BigMetric icon={Activity} label="AUM" value={vaults[0]?.aum || "Loading"}/><BigMetric icon={Gauge} label="APR / Score" value={vaults[0]?.apr || "--"} sub={`${vaults[0]?.score || "--"}/100`}/></div><Panel className="mt-5 p-5"><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.22em] text-white/40"><tr><th className="py-3">Vault</th><th>AUM</th><th>APR</th><th>Status</th><th>Leader</th><th>Score</th></tr></thead><tbody>{vaults.map((row) => <tr key={row.name} className="border-t border-white/8"><td className="py-3 font-black">{row.name}</td><td>{row.aum}</td><td>{row.apr}</td><td>{row.status}</td><td>{row.leader ? shortAddress(row.leader) : "--"}</td><td>{row.score}/100</td></tr>)}</tbody></table></div></Panel></div>;

  const flowsView = <div><ViewHeader icon={Globe2} eyebrow="ETF flows" title="HYPE ETF daily net flows" description="Daily net creations/redemptions from the Farside Hyperliquid ETF flow table. This is closer to HypurrIntel than secondary-market volume." right={<StatusPill status={flowStatus}/>} /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{flows.slice(0, 4).map((row) => <Panel key={row.ticker} className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.3em] text-white/45">{row.ticker}</p><h3 className="mt-3 text-xl font-black leading-tight">{row.name}</h3><p className="mt-1 text-sm text-white/55">{row.venue}</p></div>{row.url ? <a href={row.url} target="_blank" rel="noreferrer" className="rounded-full border border-white/12 p-2 text-white/55 hover:text-white"><ExternalLink className="h-4 w-4"/></a> : null}</div><div className="mt-7"><p className="text-xs text-white/45">Latest daily net flow</p><p className={cn("mt-2 text-3xl font-black", String(row.dollarVolume || "").startsWith("-") ? "text-red-100" : row.dollarVolume === "--" ? "text-white/50" : "text-emerald-100")}>{row.dollarVolume || "--"}</p><p className="mt-2 text-xs text-white/42">{row.status}</p></div></Panel>)}</div><Panel className="mt-5 p-5"><h2 className="text-xl font-black">Source and limitation</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-white/62">This section now uses the Farside Hyperliquid ETF Flow table, i.e. net ETF flow in US$m. It is not Yahoo trading volume. If the feed fails, the cards show -- instead of inventing data.</p></Panel></div>;

  const walletView = <div><ViewHeader icon={Wallet} eyebrow="Wallet scan" title="Hyperliquid wallet risk lookup" description="Paste any EVM address to inspect open perp positions. No wallet connect, no signatures, read-only." right={<StatusPill status={whaleStatus}/>} /><Panel className="p-5"><div className="flex flex-col gap-3 md:flex-row"><input value={wallet} onChange={(e) => setWallet(e.target.value)} placeholder="0x..." className="min-w-0 flex-1 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 outline-none placeholder:text-white/30"/><button onClick={scanWallet} className="rounded-2xl border border-cyan-200/25 bg-cyan-300/12 px-5 py-3 font-black text-cyan-100 hover:bg-cyan-300/18">Scan address</button></div><p className="mt-3 text-sm text-white/55">{whaleMessage}</p></Panel><Panel className="mt-5 p-5"><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.22em] text-white/40"><tr><th className="py-3">Coin</th><th>Side</th><th>Size</th><th>Notional</th><th>Entry</th><th>PnL</th><th>Liq</th><th>Lev</th><th>Danger</th></tr></thead><tbody>{whaleRows.map((row) => <tr key={`${row.coin}-${row.size}`} className="border-t border-white/8"><td className="py-3 font-black">{row.coin}</td><td>{row.side}</td><td>{row.size}</td><td>{row.notional}</td><td>{row.entry}</td><td className={row.rawPnl >= 0 ? "text-emerald-200" : "text-red-200"}>{row.pnl}</td><td>{row.liquidation}</td><td>{row.leverage}</td><td>{row.danger}</td></tr>)}</tbody></table></div></Panel></div>;
  ...
  return <main className="min-h-screen bg-[#031815] text-white"><div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_15%_10%,rgba(41,223,187,0.16),transparent_32rem),radial-gradient(circle_at_95%_5%,rgba(103,232,249,0.12),transparent_34rem)]"/>{sidebar}<div className="relative px-4 py-6 lg:ml-64 lg:px-8 lg:py-8"><div className="mx-auto max-w-[1500px]">{view}</div></div></main>;
}
