...
function NftSaleCard({ sale }: { sale: NftSale }) {
  ...
  return <a href={sale.url || OPENSEA_COLLECTION_URL} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-3xl border border-white/12 bg-[#062820] shadow-xl shadow-black/20 transition hover:-translate-y-0.5 hover:border-cyan-200/35">
    <div className="relative aspect-square overflow-hidden bg-[#0b332b]">
      {hasImage ? <img src={sale.image} alt={sale.name} onError={() => setBroken(true)} loading="lazy" decoding="async" referrerPolicy="no-referrer" className="h-full w-full object-cover object-left transition duration-500 group-hover:scale-[1.03]"/> : <div className="flex h-full w-full flex-col items-center justify-center gap-3 text-white/40"><ImageIcon className="h-10 w-10"/><span className="text-xs font-black uppercase tracking-[0.3em]">No image</span></div>}
    </div>
    <div className="flex min-h-20 items-center justify-between gap-3 p-4">
      ...
      <span className="shrink-0 rounded-full border border-cyan-200/20 bg-cyan-300/12 px-3 py-1 text-xs font-black text-cyan-100">{sale.price}</span>
    </div>
  </a>;
}
...
export default function App() {
  ...
  const overview = <div><ViewHeader icon={Gauge} eyebrow="HYPE investor dashboard" title="Hyperliquid market dashboard" description="A one-screen view of the metrics investors care about: price, market cap estimate, FDV, volume, open interest, funding, buyback pressure, HLP and TradFi demand." right={<><StatusPill status={marketsStatus}/><StatusPill status={buybackStatus}/><StatusPill status={flowStatus}/></>} /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><BigMetric icon={hype.rawChange >= 0 ? ArrowUpRight : ArrowDownRight} label="HYPE price" value={hype.price} change={hype.rawChange} sub={volumeToMarketCap} tone={hype.rawChange >= 0 ? "green" : "red"}/><BigMetric icon={BarChart3} label="Market cap" value={marketCapLabel} sub={`Est. ${formatNative(HYPE_CIRCULATING_SUPPLY_ESTIMATE)} circ.`}/><BigMetric icon={Zap} label="FDV" value={hype.fdv || "$--"} sub={fdvToMarketCap}/><BigMetric icon={Activity} label="24h volume" value={hype.volumeLabel} sub={volumeToOi} tone="green"/><BigMetric icon={Gauge} label="Perp open interest" value={hype.oi} sub={hypeOiShare}/><BigMetric icon={Clock3} label="Funding" value={hype.funding} sub="HYPE perp funding" tone={hype.rawFunding >= 0 ? "green" : "red"}/><BigMetric icon={Flame} label="Buyback pressure" value={buybacks.estimatedBuybackUsd24hLabel || "Loading"} sub={buybacks.estimatedBuybackHype24hLabel || "Estimated / 24h"} tone="amber"/><BigMetric icon={Layers} label="HLP AUM" value={hlpVault?.aum || "Loading"} sub={hlpVault?.apr ? `${hlpVault.name} · ${hlpVault.apr}` : "Primary vault"}/></div><div className="mt-5 grid gap-5 xl:grid-cols-[1.2fr_0.8fr]"><Panel className="p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-black">HYPE 24h price action</h2><p className="text-sm text-white/55">Live candles when available. The badge gives the 24h move.</p></div><MoveBadge value={hype.rawChange}/></div><div className="h-[320px] rounded-3xl border border-white/8 bg-black/18 p-4"><Sparkline points={history.map((point) => point.price)} height={300} stroke={hype.rawChange >= 0 ? "#6ee7b7" : "#fda4af"}/></div><div className="mt-4 grid gap-3 md:grid-cols-4"><Panel className="p-4"><p className="text-xs font-black uppercase tracking-[0.24em] text-white/45">Risk heat</p><p className="mt-3 text-2xl font-black">{hype.risk}/100</p><div className="mt-3 h-2 rounded-full bg-white/10"><div className="h-full rounded-full bg-cyan-200" style={{ width: `${hype.risk}%` }}/></div></Panel><Panel className="p-4"><p className="text-xs font-black uppercase tracking-[0.24em] text-white/45">Total perp OI</p><p className="mt-3 text-2xl font-black">{formatUsd(totalOi)}</p><p className="text-xs text-white/55">all tracked markets</p></Panel><Panel className="p-4"><p className="text-xs font-black uppercase tracking-[0.24em] text-white/45">Total volume</p><p className="mt-3 text-2xl font-black">{formatUsd(totalVolume)}</p><p className="text-xs text-white/55">24h notional</p></Panel><Panel className="p-4"><p className="text-xs font-black uppercase tracking-[0.24em] text-white/45">Supply</p><p className="mt-3 text-2xl font-black">1.00B</p><p className="text-xs text-white/55">max FDV base</p></Panel></div></Panel><Panel className="p-5"><div className="mb-4"><h2 className="text-xl font-black">Investor pulse</h2><p className="text-sm text-white/55">Compact demand and leverage signals, no NFT noise.</p></div><div className="space-y-3"><div className="rounded-2xl border border-white/8 bg-white/[0.04] p-4"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-white/45">Assistance Fund pressure</p><p className="mt-2 text-2xl font-black">{buybacks.estimatedBuybackUsd24hLabel || "--"}</p></div><Flame className="h-6 w-6 text-amber-200"/></div><p className="mt-2 text-sm text-white/55">{buybacks.estimatedBuybackHype24hLabel || "Estimated HYPE / 24h"}</p></div><div className="rounded-2xl border border-white/8 bg-white/[0.04] p-4"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-white/45">HYPE TWAP net</p><p className="mt-2 text-2xl font-black">{twapSummary?.netSide || "--"} {twapSummary?.netLabel || "$--"}</p></div><Clock3 className="h-6 w-6 text-cyan-100"/></div><p className="mt-2 text-sm text-white/55">Buy {twapSummary?.buyLabel || "$--"} · Sell {twapSummary?.sellLabel || "$--"}</p></div><div className="rounded-2xl border border-white/8 bg-white/[0.04] p-4"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-white/45">ETF / ETP tape</p><p className="mt-2 text-2xl font-black">{etfHeadline}</p></div><Globe2 className="h-6 w-6 text-emerald-100"/></div><p className="mt-2 text-sm text-white/55">{etfHeadlineRow.ticker} · {etfHeadlineRow.status}</p></div></div></Panel></div><div className="mt-5 grid gap-5 xl:grid-cols-2"><Panel className="p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-black">Top markets by 24h volume</h2><p className="text-sm text-white/55">Where the Hyperliquid flow is concentrated now.</p></div><Pill>{topVolumeMarkets.length} markets</Pill></div><div className="space-y-2">{topVolumeMarkets.map((row, index) => <div key={`vol-${row.symbol}`} className="flex items-center justify-between rounded-2xl border border-white/8 bg-white/[0.04] p-3"><div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-cyan-200/10 text-xs font-black text-cyan-100">{index + 1}</span><div><p className="font-black">{row.symbol}</p><p className={cn("text-xs", row.rawChange >= 0 ? "text-emerald-200" : "text-red-200")}>{row.change}</p></div></div><div className="text-right"><p className="font-black">{row.volumeLabel}</p><p className="text-xs text-white/45">24h volume</p></div></div>)}</div></Panel><Panel className="p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-black">Top markets by open interest</h2><p className="text-sm text-white/55">Leverage concentration and liquidation risk proxy.</p></div><Pill>{formatUsd(totalOi)} OI</Pill></div><div className="space-y-2">{topOiMarkets.map((row, index) => <div key={`oi-${row.symbol}`} className="flex items-center justify-between rounded-2xl border border-white/8 bg-white/[0.04] p-3"><div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-white/8 text-xs font-black text-white/70">{index + 1}</span><div><p className="font-black">{row.symbol}</p><p className="text-xs text-white/45">Risk {row.risk}/100</p></div></div><div className="text-right"><p className="font-black">{row.oi}</p><p className="text-xs text-white/45">{totalOi ? ((row.rawOi / totalOi) * 100).toFixed(1) : "--"}% share</p></div></div>)}</div></Panel></div></div>;
  ...
  const flowsView = <div><ViewHeader icon={Globe2} eyebrow="TradFi ETF volumes" title="HYPE ETF / ETP daily volume" description="Daily trading volume and estimated value traded for listed HYPE products. This is volume, not net inflow; exact primary-market creations/redemptions require issuer/AP data." right={<StatusPill status={flowStatus}/>} /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{flows.slice(0, 4).map((row) => <Panel key={row.ticker} className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.3em] text-white/45">{row.ticker}</p><h3 className="mt-3 text-xl font-black leading-tight">{row.name}</h3><p className="mt-1 text-sm text-white/55">{row.venue}</p></div>{row.url ? <a href={row.url} target="_blank" rel="noreferrer" className="rounded-full border border-white/12 p-2 text-white/55 hover:text-white"><ExternalLink className="h-4 w-4"/></a> : null}</div><div className="mt-6 grid grid-cols-2 gap-3"><div><p className="text-xs text-white/45">Daily volume</p><p className="mt-1 text-lg font-black">{row.volume || "--"}</p></div><div><p className="text-xs text-white/45">Value traded</p><p className="mt-1 text-lg font-black">{row.dollarVolume || "--"}</p></div><div><p className="text-xs text-white/45">Price</p><p className="mt-1 font-black">{row.price || "--"}</p></div><div><p className="text-xs text-white/45">24h</p><p className={cn("mt-1 font-black", String(row.change || "").startsWith("-") ? "text-red-200" : "text-emerald-200")}>{row.change || "--"}</p></div></div><p className="mt-5 text-xs text-white/42">{row.status}</p></Panel>)}</div><Panel className="mt-5 p-5"><h2 className="text-xl font-black">Read this correctly</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-white/62">This section shows exchange trading volume. It is useful for demand/liquidity monitoring, but it is not the same thing as net fund inflow. A true daily inflow table needs issuer holdings/AUM history or a dedicated flow feed.</p></Panel></div>;
