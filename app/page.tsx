...
type NftSale = { id: string; name: string; price: string; usd?: string; time: string; image?: string; url?: string; priceVerified?: boolean; priceSource?: string };
...
type FlowRow = { name: string; ticker: string; venue: string; price?: string; change?: string; volume?: string; dollarVolume?: string; dailyFlow?: string; cumulativeFlow?: string; aum?: string; fee?: string; status: string; url?: string; updatedAt?: string };
...
const FALLBACK_FLOWS: FlowRow[] = [
  { name: "Bitwise Hyperliquid ETF", ticker: "BHYP", venue: "US spot HYPE", status: "Waiting for Farside flow feed", dailyFlow: "Loading" },
  { name: "21Shares Hyperliquid ETF", ticker: "THYP", venue: "US spot HYPE", status: "Waiting for Farside flow feed", dailyFlow: "Loading" },
  { name: "Grayscale HYPE Staking ETF", ticker: "HYPG", venue: "US staking HYPE", status: "Waiting for Farside flow feed", dailyFlow: "Loading" },
  { name: "Total HYPE ETF net flow", ticker: "TOTAL", venue: "All tracked HYPE ETFs", status: "Waiting for Farside flow feed", dailyFlow: "Loading" },
];
...
const NAV_ITEMS: NavItem[] = [
  ...
  { id: "flows", label: "ETF flows", description: "Daily net flow", icon: Globe2 },
...
function normalizeNftSale(raw: any, index: number): NftSale {
  ...
  const price = typeof raw.price === "string" ? raw.price : raw.priceLabel || "Sale price unavailable";
  return {
    id,
    name,
    price,
    priceVerified: Boolean(raw.priceVerified),
    priceSource: raw.priceSource || "unknown",
...
function NftSaleCard({ sale }: { sale: NftSale }) {
  ...
  const verified = Boolean(sale.priceVerified && sale.price && !sale.price.startsWith("--") && !sale.price.toLowerCase().includes("unavailable"));
  return <a href={sale.url || OPENSEA_COLLECTION_URL} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-3xl border border-white/12 bg-[#062820] shadow-xl shadow-black/20 transition hover:-translate-y-0.5 hover:border-cyan-200/35">
    <div className="relative aspect-square overflow-hidden bg-[#0b332b]">
      {hasImage ? <img src={sale.image} alt={sale.name} onError={() => setBroken(true)} loading="lazy" decoding="async" referrerPolicy="no-referrer" className="h-full w-full object-cover object-center transition duration-500 group-hover:scale-[1.03]"/> : <div className="flex h-full w-full flex-col items-center justify-center gap-3 text-white/40"><ImageIcon className="h-10 w-10"/><span className="text-xs font-black uppercase tracking-[0.3em]">No image</span></div>}
    </div>
    <div className="flex min-h-20 items-center justify-between gap-3 p-4">
      ...
      <span className={cn("shrink-0 rounded-full border px-3 py-1 text-xs font-black", verified ? "border-cyan-200/20 bg-cyan-300/12 text-cyan-100" : "border-white/10 bg-white/[0.04] text-white/45")}>{verified ? sale.price : "Price n/a"}</span>
    </div>
  </a>;
}
...
export default function App() {
  ...
  const buyTwaps = twaps.filter((row) => row.side === "Buy").slice(0, 8);
  const sellTwaps = twaps.filter((row) => row.side === "Sell").slice(0, 8);
  const twapColumn = (title: string, rows: TwapRow[], side: "Buy" | "Sell") => (
    <Panel className="p-5">
      <div className="mb-4 flex items-center justify-between">
        <h2 className={cn("text-xl font-black", side === "Buy" ? "text-emerald-100" : "text-red-100")}>{title}</h2>
        <Pill className={side === "Buy" ? "text-emerald-100" : "text-red-100"}>{rows.length} clusters</Pill>
      </div>
      <div className="space-y-3">
        {rows.length ? rows.map((row, index) => (
          <div key={`${side}-${index}-${row.notional}`} className="rounded-2xl border border-white/8 bg-white/[0.04] p-4">
            <div className="flex items-center justify-between gap-3">
              <span className="text-sm text-white/55">{row.slices} slices · {row.confidence}</span>
              <span className="text-lg font-black">{row.notional}</span>
            </div>
            <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-white/55">
              <span>{row.size}</span>
              <span>{row.avgPrice}</span>
              <span>{row.lastTrade}</span>
            </div>
          </div>
        )) : <p className="rounded-2xl border border-white/8 bg-white/[0.04] p-4 text-sm text-white/55">No strong {side.toLowerCase()} TWAP-like cluster in the latest public HYPE trades.</p>}
      </div>
    </Panel>
  );

  const twapView = <div><ViewHeader icon={Clock3} eyebrow="HYPE TWAP" title="HYPE buyer/seller TWAP tape" description="HYPE-only live tape: public recent trades split into buyer and seller execution clusters. Exact exchange-native global TWAPs need an indexer; this is a transparent public-data approximation." right={<StatusPill status={twapStatus}/>} /><div className="grid gap-4 md:grid-cols-3"><BigMetric icon={ArrowUpRight} label="Buyer flow 10m" value={twapSummary?.buyLabel || "$--"} sub="public recent trades" tone="green"/><BigMetric icon={ArrowDownRight} label="Seller flow 10m" value={twapSummary?.sellLabel || "$--"} sub="public recent trades" tone="red"/><BigMetric icon={Activity} label="Net flow" value={`${twapSummary?.netSide || "--"} ${twapSummary?.netLabel || "$--"}`} sub="10m window"/></div><div className="mt-5 grid gap-5 xl:grid-cols-2">{twapColumn("Buyer TWAPs", buyTwaps, "Buy")}{twapColumn("Seller TWAPs", sellTwaps, "Sell")}</div><Panel className="mt-5 p-5"><h2 className="mb-4 text-xl font-black">Recent HYPE prints</h2><div className="max-h-[440px] overflow-auto rounded-2xl border border-white/8"><table className="w-full text-left text-sm"><thead className="sticky top-0 bg-[#06231f] text-xs uppercase tracking-[0.18em] text-white/40"><tr><th className="p-3">Side</th><th>Price</th><th>Size</th><th>Notional</th><th>Time</th></tr></thead><tbody>{trades.slice(0, 36).map((row) => <tr key={row.id} className="border-t border-white/8"><td className={cn("p-3 font-black", row.side === "Buy" ? "text-emerald-200" : "text-red-200")}>{row.side}</td><td>{row.price}</td><td>{row.size}</td><td>{row.notionalLabel}</td><td className="text-white/45">{row.timeLabel}</td></tr>)}</tbody></table></div></Panel></div>;
  ...
  const latestTotalFlow = flows.find((row) => row.ticker === "TOTAL") || flows[flows.length - 1];
  const issuerFlows = flows.filter((row) => row.ticker !== "TOTAL");
  const flowsView = <div><ViewHeader icon={Globe2} eyebrow="TradFi ETF flows" title="HYPE ETF daily net flows" description="Daily net creations/redemptions for HYPE ETFs when the public Farside feed is reachable. This is the metric that matters more than simple exchange volume." right={<StatusPill status={flowStatus}/>} /><div className="grid gap-4 md:grid-cols-4"><BigMetric icon={Globe2} label="Latest total flow" value={latestTotalFlow?.dailyFlow || "--"} sub={latestTotalFlow?.updatedAt || latestTotalFlow?.status || "Latest row"} tone={String(latestTotalFlow?.dailyFlow || "").startsWith("-") ? "red" : "green"}/>{issuerFlows.slice(0, 3).map((row) => <BigMetric key={row.ticker} icon={Activity} label={row.ticker} value={row.dailyFlow || row.dollarVolume || "--"} sub={row.status} tone={String(row.dailyFlow || "").startsWith("-") ? "red" : "cyan"}/>)}</div><Panel className="mt-5 p-5"><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.22em] text-white/40"><tr><th className="py-3">Fund</th><th>Issuer</th><th>Latest daily net flow</th><th>Cumulative</th><th>Status</th><th>Source</th></tr></thead><tbody>{flows.map((row) => <tr key={row.ticker} className="border-t border-white/8"><td className="py-3 font-black">{row.ticker}</td><td>{row.name}</td><td className={cn("font-black", String(row.dailyFlow || "").startsWith("-") ? "text-red-200" : "text-emerald-200")}>{row.dailyFlow || row.dollarVolume || "--"}</td><td>{row.cumulativeFlow || "--"}</td><td className="text-white/55">{row.status}</td><td>{row.url ? <a href={row.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-cyan-100">Open <ExternalLink className="h-3.5 w-3.5"/></a> : "--"}</td></tr>)}</tbody></table></div></Panel><Panel className="mt-5 p-5"><h2 className="text-xl font-black">Important</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-white/62">This page is now built around ETF net flows, not just Yahoo trading volume. It uses the public Farside HYPE flow table when available. If the feed is unreachable, the site shows unavailable data instead of inventing numbers.</p></Panel></div>;
