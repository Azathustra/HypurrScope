...
export function derivePrimaryAction(input: PrimaryActionInput): { label: string; disabled: boolean; reason: string; action: PrimaryAction; secondary?: string } {
  ...
  if (!input.ticketValid) return { label: "Fix ticket", disabled: true, reason: "Ticket has validation errors.", action: "fix_ticket" };
  ...
  if (!input.walletConnected) return { label: "Preview ticket", disabled: false, reason: "Wallet is not connected, so this stays preview-only.", action: "preview" };
  if (!input.realExecutionEnabled) return { label: "Copy ticket", disabled: false, reason: "Real execution is disabled by feature flag.", action: "copy_ticket", secondary: "Open on Hyperliquid" };
  if (input.builderEnabled && !input.builderConfigValid) return { label: "Fix builder config", disabled: true, reason: "Builder config is invalid.", action: "fix_ticket" };
